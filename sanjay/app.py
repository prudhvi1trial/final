...
# Import Viven Logic
try:
    from viven_logic.pose_detector import PoseDetector as VivenPoseDetector
    from viven_logic.filters import FILTER_REGISTRY as VIVEN_FILTER_REGISTRY
except ImportError:
    VivenPoseDetector = None
    VIVEN_FILTER_REGISTRY = {}
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # self.switch_gesture_control.select()  # disabled by default
        ...
        self.bg_menu = ctk.CTkOptionMenu(self.tabview.tab("Appearance"), values=["Solid Black", "Transparent (Camera)", "Room Background"])
        ...
        # Initialize Viven Detector
        self.viven_detector = None
        if VivenPoseDetector:
            viven_model = os.path.join(base_path, "viven_logic", "pose_landmarker_lite.task")
            self.viven_detector = VivenPoseDetector(model_path=viven_model)
        ...
        self.bg_image_path = os.path.join(base_path, "assets", "room_bg.jpg")
        self.bg_image_raw = cv2.imread(self.bg_image_path) if os.path.exists(self.bg_image_path) else None
        
        # Magic Button Assets
        self.btn_image_path = os.path.join(base_path, "assets", "red_button.png")
        self.btn_image_raw = cv2.imread(self.btn_image_path, cv2.IMREAD_UNCHANGED) if os.path.exists(self.btn_image_path) else None
        
        self.sparkle_image_path = os.path.join(base_path, "assets", "blue_sparkle.png")
    ...
    def draw_stickman(self, img, lm_list):
        ...
        # Check if it's a Viven Filter
        if style in VIVEN_FILTER_REGISTRY and VIVEN_FILTER_REGISTRY[style] is not None:
            if hasattr(self, 'last_pose_viven') and self.last_pose_viven:
                pose_viven = self.last_pose_viven
                # Viven filters handle their own background/canvas usually
                # But we want to respect Sanjay's background system
                if self.bg_menu.get() == "Room Background" and self.bg_image_raw is not None:
                ...
                try:
                    ...
                    if duration > 100: # Log only if significantly slow
                        print(f"Warning: Filter {style} is slow ({duration:.1f}ms)")
                    return result
                except Exception as e:
                    print(f"Error applying Viven filter {style}: {e}")
                    return art_img # Fallback
            else:
                art_img = np.zeros((h, w, c), dtype=np.uint8) # Fail safe
                return art_img
        elif style == "Minimalist Line Art":
        ...
        elif (style in ["Shadow Void", "Magic Button"] or self.bg_menu.get() == "Room Background") and self.bg_image_raw is not None:
            art_img = cv2.resize(self.bg_image_raw, (w, h))
            ...
            # Fallback to darkened camera background (Transparent / Photo Fail)
            art_img = cv2.convertScaleAbs(img, alpha=0.3, beta=0) 
    ...
    def detect_gesture(self, lm_list):
        ...
        if 15 in points and 16 in points:
            ...
            # TRIGGER: Both hands above reference (nose/shoulders) with small buffer
            if lw_y < ref_y - 20 and rw_y < ref_y - 20: 
                current_time = time.time()
                if current_time - self.last_style_change_time > 2.0: 
                    self.change_style(1)
    ...
    def update_video(self):
        ...
        if success:
            ...
            elif style in VIVEN_FILTER_REGISTRY and VIVEN_FILTER_REGISTRY[style] is not None:
                # Viven Filter active
                img_processed = img.copy()
                timestamp_ms = int(time.time() * 1000)
                if self.viven_detector:
                    pose_viven = self.viven_detector.detect(img, timestamp_ms)
                    # Convert Viven landmarks to Sanjay's lm_list for gesture support
                    lm_list = [[p_id, x, y] for p_id, (x, y) in pose_viven.landmarks.items() if p_id < 33]
                    # Store pose_viven for draw_stickman to reuse
                    self.last_pose_viven = pose_viven
                else:
                    lm_list = []
                self.holistic_face_results = None
                self.holistic_hand_results = None
